function fun(ResultTable, Data, resolution, special)
  ...
  % Make plot pretty
  set(gca,'ydir','normal')
  subplot_title = sprintf('p=%.0f, r=%.2f', p, r);
  title({'Early G1',subplot_title})
  legend(gca,'off');
  xlabel('');ylabel({'STRADa Average Intensity','Nuclear / Cytoplasm'});
  ...
  title(subplot_title)
  legend(gca,'off');
  xlabel('');ylabel({'STRADa Average Intensity','Nuclear / Total'});
  xlim([prctile(cell_size, 1) prctile(cell_size, 99)]);
  ...
  title(subplot_title)
  legend(gca,'off');
  xlabel('Cell Size (area)');ylabel({'STRADa Average Intensity','Cytoplasm / Total'});
  xlim([prctile(cell_size, 1) prctile(cell_size, 99)]);
  ...
  filename = sprintf('cell_cycle_nuc_cyto_cellsize_JPD_%s.png',special);
