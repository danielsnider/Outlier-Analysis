function fun(ResultTable, Data)
  ...
  for cell_id=random_cells
    ...
    filename = sprintf('Z:/DanielS/Projects/Eden_STRADa/Plots/Analysis 6 - normalized by total STRADa instead of SE/single cell images/by ratio/ratio-%.5f_stage-%s_mass-%s_id-%s.png',STRADa_ratio(cell_id),char(cell_stage),num2str(cell_mass(cell_id)),num2str(cell_id));
    export_fig(filename, '-m1')
    filename = sprintf('Z:/DanielS/Projects/Eden_STRADa/Plots/Analysis 6 - normalized by total STRADa instead of SE/single cell images/by stage/stage-%s_ratio-%.5f_mass-%s_id-%s.png',char(cell_stage),STRADa_ratio(cell_id),num2str(cell_mass(cell_id)),num2str(cell_id));
    export_fig(filename, '-m1')
    filename = sprintf('Z:/DanielS/Projects/Eden_STRADa/Plots/Analysis 6 - normalized by total STRADa instead of SE/single cell images/by mass/mass-%s_ratio-%.5f_stage-%s_id-%s.png',num2str(cell_mass(cell_id)),STRADa_ratio(cell_id),char(cell_stage),num2str(cell_id));
    export_fig(filename, '-m1')
  end
end