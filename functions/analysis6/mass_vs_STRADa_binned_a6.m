 function fun(ResultTable, Data)
  ...
  aspects(n).title = 'Cell Mass vs STRADa localization';
  aspects(n).ylabel = 'Cell Mass (SE)';
  aspects(n).xlabel = '% of STRADa in Nucleus';
  aspects(n).filename = sprintf('Z:/DanielS/Projects/Eden_STRADa/Plots/Analysis 6 - normalized by total STRADa instead of SE/%s_vs_%s_bins%s_hidepoints_%s.png', aspects(n).X,aspects(n).Y,num2str(aspects(n).num_bins),string(aspects(n).hide_points));
  aspects(n).resolution = [2          42        1191        1074];
  n=n+1;

  %List of aspects to be visualized
  aspects(n).num_bins = 19;
  aspects(n).X = 'STRADa_ratio';
  aspects(n).Y = 'cell_mass';
  aspects(n).hide_points = true;
  aspects(n).title = 'Cell Mass vs STRADa localization';
  ...
  aspects(n).ylabel = '% of STRADa in Nucleus';
  aspects(n).filename = sprintf('Z:/DanielS/Projects/Eden_STRADa/Plots/Analysis 6 - normalized by total STRADa instead of SE/%s_vs_%s_bins%s_hidepoints_%s.png', aspects(n).X,aspects(n).Y,num2str(aspects(n).num_bins),string(aspects(n).hide_points));
  aspects(n).resolution = [2          42        1191        1074];
  n=n+1;
